...
An N-way switch: `select` (a 1-based integer) picks a `value_N` input, and
only that branch is ever evaluated -- every other branch's whole upstream
chain never runs. Like `H3GateAB`, it raises a `ValueError` naming the
selection rather than silently falling back to another branch if the
selected input isn't connected.

The input sockets grow dynamically, the same way `H3CombineLatents`' do
(paired `web/gate_switch.js`, mirroring `web/combine_latents.js`): connecting
the last `value_N` socket adds a fresh empty one after it, and disconnecting
